...
const events = require('events'),
      ...
      fs     = require('fs'),
      path   = require('path'),
      {name} = require(path.join(__dirname, '..', 'package.json')),
...
try {
  debug = require('debug')('wssip:menu');
} catch(e) {
  debug = function() {}
}
    ...
    submenu: [{
      ...
      click: (item, focusedWindow) => {
        dialog.showOpenDialog(focusedWindow, {
          ...
          focusedWindow.webContents.send('wssip-open', filePaths);
        });
      }
    }, {
      label: 'Save',
      ...
      click: (item, focusedWindow) => {
        dialog.showSaveDialog(focusedWindow, {
          ...
          focusedWindow.webContents.send('wssip-save', filename);
        });
      }
    }, {
      label: 'Save As...',
      ...
      click: (item, focusedWindow) => {
        dialog.showSaveDialog(focusedWindow, {
          ...
          focusedWindow.webContents.send('wssip-saveas', filename);
        });
      }
    }, {
      type: 'separator'
