...
module.exports = class WebSocketProxy {
  ...
  start() {
    debug('setting new instance of mitmInst');

    let perMsgDeflate = argv.disableDeflate ? true : config.get('perMessageDeflate');

    let options = {
      hostname: ifExists(argv.host, config.get('proxyHost')),
      port: ifExists(argv.port, config.get('proxyPort')),
      tmpDir: this.tmpDir,
      useUpstreamProxy: ifExists(argv.upstream, config.get('useUpstreamProxy')),
      proxyHostname: ifExists(argv.upstreamHost, config.get('upstreamHost')),
      proxyPort: ifExists(argv.upstreamPort, config.get('upstreamPort')),
      timeout: ifExists(argv.timeout, config.get('timeout')),
    ...
    this.mitmInst.on('beforeListening', (httpProxy) => {
      let wsServer = new WebSocket.Server({
        server: httpProxy,
        rejectUnauthorized: ifExists(argv.rejectUnauthorized, config.get('rejectUnauthorized')),
        perMessageDeflate: perMsgDeflate
      });

      wsServer.on('connection', this.connection);
    });

    this.mitmInst.on('afterListening', (proxy) => {
      let title = `WSSiP/${version} - listening on `;
      title += options.hostname + `:${options.port}`;

      if(options.useUpstreamProxy === true) {
        title += ` -> ${options.proxyHostname}:${options.proxyPort}`;
      }
  ...
  connection(cSocket) {
    ...
    this.serverSocket[id].on('open', () => {
      ...
      ipcMain.on(`mitmengine-send-${parseInt(id)}`, (e, sender, type, data, flags) => {
        if('binary' in flags && flags.binary === true && typeof data === 'string' && data !== '') {
          if(/^[a-fA-F0-9]/.test(str)) {
            data = Buffer.from(data, "hex");
          } else {
            this.error(sender, new Error('Message is not in hex format for conversion to binary'));
            return;
          }
        }
