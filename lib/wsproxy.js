...
try {
  debug = require('debug')('wssip:wsproxy');
} catch(e) {
  debug = function() {}
}
...
module.exports = class WebSocketProxy {
  ...
  constructor(browserWindow) {
    this.tmpDir = path.join(app.getPath('userData'), 'certstore');
  ...
  start() {
    ...
    this.mitmInst = new mitmengine({
      hostname: config.get('proxyHost'),
      port: config.get('proxyPort'),
      tmpDir: this.tmpDir,
      useUpstreamProxy: config.get('useUpstreamProxy'),
      proxyHostname: config.get('upstreamHost'),
      proxyPort: config.get('upstreamPort'),
    ...
    this.mitmInst.on('beforeListening', (httpProxy) => {
      let wsServer = new WebSocket.Server({
        server: httpProxy,
        rejectUnauthorized: config.get('rejectUnauthorized'),
        perMessageDeflate: config.get('perMessageDeflate')
      });
    ...
    this.mitmInst.on('afterListening', (proxy) => {
      ...
      title += config.get('proxyHost') + `:${config.get('proxyPort')}`;

      if(config.get('useUpstreamProxy') === true) {
        title += ` -> ${config.get('upstreamHost')}:${config.get('upstreamPort')}`;
      }
