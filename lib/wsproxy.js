...
module.exports = class WebSocketProxy {
  ...
  stop() {
    this.BrowserWindow.setTitle(`WSSiP/${version} - stopping...`);
    ...
    this.mitmInst.on('close', () => {
      menuBuilder(this);
      this.BrowserWindow.setTitle(`WSSiP/${version} - not listening`);
    });
  ...
  resetCAs() {
    return new Promise((resolve, reject) => {
      fs.unlink(path(this.tmpDir), e => {
        if(e) {
          reject(e);
          return;
        }

        this.mitmInst._cacheMasterKeys()
  ...
  connection(cSocket) {
    ...
    let connectionUrl = url.parse(this.connections[id].client.upgradeReq.url),
        connectionHeaders = Object.assign({}, this.connections[id].client.upgradeReq.headers);
    ...
    this.connections[id].server = new WebSocket(properUrl, connectionArgs);
