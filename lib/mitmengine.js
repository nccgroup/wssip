...
module.exports = class mitmengine extends events.EventEmitter {
  ...
  onConnect(request, clientSocket, head) {
    ...
    if (is_https) {
      ...
      //})();
      this.setupHttpsServer(parsedUrl)
  ...
  _curlRequest(method, url, httpVersion, headers, postField, clientResponse) {
    ...
    curl.on('header', c => clientResponse.write(c));
  ...
  _nodeRequest(options, _buf, _size, clientResponse, connectReq) {
    (options.protocol === 'https:' ? https : http).request(options, serverResponse => {
      ...
      clientResponse.end(this._writeErrorPage(err));
    }).write(Buffer.concat(_buf, _size));
  }
  ...
  onRequest(clientRequest, clientResponse) {
    ...
    clientRequest.on('end', () => {
      debug(`${this.useCurl ? 'CURL' : 'NODE'} REQUEST: ${clientRequest.method} ${clientRequest.url}`);
      ...
      if (this.useCurl === true) {
        ...
        this._curlRequest(clientRequest.method, clientRequest.url, clientRequest.httpVersion, headers, _postField, clientResponse);
      } else {
        let options = parse(clientRequest.url);

        options.headers = this._getRawHeaderObj(clientRequest.rawHeaders);
        ...
        if (this.useUpstreamProxy === true) {
          ...
          connectReq.on('connect', (res, socket, head) => {
            ...
            this._nodeRequest(options, _buf, _size, clientResponse, connectReq);
          });
          ...
          this._nodeRequest(options, _buf, _size, clientResponse, null);
        }
      }
    });
