...
defaultSet('useUpstreamProxy', argv.u);
...
ipcMain.on('changeUpstreamPortCallback', (e, port) => setPort('upstreamPort', port));
...
process.on('uncaughtException', err => {
  if(err.code === 'EADDRINUSE' && mainWindow !== null) return;

  dialog.showErrorBox(err.message, `${err.stack.replace(err.message, '')}`);
  process.exit(1);
});
